/**
 * VTT - Fetch and parse subtitle text and timing from VTT files
 * Consolidated: fetch + parse (from 01_load)
 */

// ============================================================================
// Netflix Subtitle Script Injection
// ============================================================================
 ...
 * 
 * Subtitle Flow: Listens for SMARTSUBS_GET_SUBTITLES_PAGE messages → accesses window.netflix.appContext.state.playerApp.getAPI().videoPlayer.getVideoPlayerBySessionId(sessionIds[0])
 * Gets subtitle tracks and returns them via postMessage
 */
export const NETFLIX_SUBTITLE_INJECT_CODE = `
(function() {
  'use strict';
...
let subtitleScriptInjectionPromise = null;

export async function injectNetflixSubtitleScript() {
 ...
 * @returns {Promise<object|null>} {content, mediaId} or null if Thai not available
 */
export async function fetchThaiVTTContent(mediaId) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const messageHandler = (event) => {
      if (!event.data || event.data.type !== 'SMARTSUBS_SUBTITLES_RESPONSE') {
        return;
      }
...
/**
 * ENGLISH: Fetch English VTT from Netflix
 * Only fetches English subtitles (no fallback)
 * @param {string} mediaId - Media ID
 * @returns {Promise<object|null>} {content, mediaId} or null if English not available
 */
export async function fetchEnglishVTTContent(mediaId) {
  const resolvedMediaId = mediaId;
  if (!resolvedMediaId) {
    throw new Error('Could not identify video - mediaId required');
  }
  ...
  return new Promise((resolve, reject) => {
    const timeout = setTimeout(() => {
      console.warn('[fetchEnglishVTTContent] Timeout waiting for subtitle tracks response (10s)');
      window.removeEventListener('message', messageHandler);
      resolve(null);
    }, 10000);
    ...
    const messageHandler = (event) => {
      ...
      if (event.data.error) {
        console.error('[fetchEnglishVTTContent] Error from injected script:', event.data.error);
      }
      
      // Filter for English ('en') language, exclude CC
      const englishTrack = tracks.find(track => {
        const langLower = (track.langCode || '').toLowerCase();
        return (langLower.includes('en') || langLower.includes('english')) && !track.isCC;
      });
      
      if (!englishTrack || !englishTrack.url) {
        console.warn('[fetchEnglishVTTContent] No English track found (excluding CC)');
        resolve(null);
        return;
      }
      ...
      fetch(englishTrack.url)
        .then(response => {
          if (!response.ok) {
            console.warn('[fetchEnglishVTTContent] VTT fetch failed:', response.status, response.statusText);
            resolve(null);
            return;
          }
          return response.text();
        ...
        .then(content => {
          if (content) {
            ...
            console.warn('[fetchEnglishVTTContent] VTT fetch returned empty content');
            resolve(null);
          }
        })
        .catch((error) => {
          console.error('[fetchEnglishVTTContent] VTT fetch error:', error);
          resolve(null);
        });
    };
...
/**
 * THAI: Parse timestamp string from Thai VTT content
 ...
 */
export function parseThaiVTTTimestamp(timestamp) {
...
/**
 * ENGLISH: Parse timestamp string from English VTT content
 * Converts "HH:MM:SS.mmm" format to seconds
 * @param {string} timestamp - Timestamp string (e.g., "00:00:01.234")
 * @returns {number|null} Seconds as float, or null if invalid
 */
export function parseEnglishVTTTimestamp(timestamp) {
  const parts = timestamp.trim().split(':');
  if (parts.length !== 3) return null;
...
/**
 * THAI: Parse complete Thai VTT file content
 * Stores text in `thai` field
 * @param {string} content - VTT file content
 * @returns {Array} Array of subtitle objects with thai field
 */
export function parseThaiVTTContent(content) {
  const subtitles = [];
  ...
  for (const block of blocks) {
    ...
    const startTime = parseThaiVTTTimestamp(startTimestamp);
    const endTime = parseThaiVTTTimestamp(endTimestampMatch[1]);
    if (startTime === null || endTime === null) continue;
    
    const textLines = lines.slice(2);
    const text = textLines.map(line => stripHTMLTags(line)).filter(line => line.trim()).join('\n').trim();
    if (!text) continue;
    ...
    subtitles.push({
      ...
      text,
      thai: text,
      startSecThai: String(startTime),
      endSecThai: String(endTime)
    });
  }
...
/**
 * ENGLISH: Parse complete English VTT file content
 * Stores text in `english` field
 * @param {string} content - VTT file content
 * @returns {Array} Array of subtitle objects with english field
 */
export function parseEnglishVTTContent(content) {
  const subtitles = [];
  const blocks = content.trim().split(/\n\s*\n/);
  ...
  for (const block of blocks) {
    ...
    const startTime = parseEnglishVTTTimestamp(startTimestamp);
    const endTime = parseEnglishVTTTimestamp(endTimestampMatch[1]);
    if (startTime === null || endTime === null) continue;
    ...
    subtitles.push({
      ...
      text,
      english: text,
      startSecEng: String(startTime),
      endSecEng: String(endTime)
    });
  }
